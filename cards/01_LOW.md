...
- **NO ROUNDED SAUSAGES — and the default IS a sausage.** One radius written at both ends of a chain is a tube, and a creature assembled from tubes is a balloon animal. The engine BLOCKs it (`soft_mass`) and states the fix in numbers. The ruler is FORM: the share of a volume's walls that lean off its own bone by 8° or more, floor 10%. A tube of one radius scores 0 whatever its tessellation; a chest that swells and a waist that narrows, a thigh that tapers into a hock, a muzzle that steps down from the brow score 30-60%. **The lever is the PROFILE ON THE VOLUME**: rows 15%+ apart in radius over a fifth of the chain, `exp`/`bias` where the section turns boxy or keeled. `smooth_angle` and `sides` do NOT move it — they set how the walls are shaded, and dropping `smooth_angle` under the wall angle facets an organic body into planar strips (that was the old ruler's lever, and it was the wrong one). A `sharp` row is the SILHOUETTE lever and does nothing unless the radius steps across it. A mass genuinely meant to be a smooth lump declares `"soft": true`. Fur that must leave the silhouette — a ruff, a mane, a bushy tail — is a `tufts` part, not a fatter ring.
...
- `blob` — a whole FLANK whose in-between azimuths close into a lump their neighbours do not: both obliques on one side (az045 and az135, or az315 and az225) have at most 0.68x the outline bites (convexity deficiency, on the orbit's shared window at 96 and 48 px) of the smaller of their two neighbours. az000/az180 are never judged: end-on is compact by nature. A pose built for the front and the side camera collapses exactly here — arms folded into the torso at 45°, legs stacked behind each other. It no longer counts protrusions: one ear or a softer tail brush used to flip a view, and it cannot see a creature that is a lump from EVERY side (that is `soft_mass` and the reader's job).
