...
9. **One creature, one agent. The reader is the ONLY subagent.** You build the whole thing yourself — brief, skeleton, parts, anims, repairs — in one session, and you spawn subagents for exactly one purpose: the context-free reads of law 2. Handing the modelling itself to a chain of subagents feels tidy and costs more on every axis — tokens, wall clock and turns alike. Onboarding is not the cause; every fresh agent reads the same ground truth. The cause is RE-DERIVATION: work the previous agent had already done and could not hand over. The reader splits cheaply for the opposite reason — it is one turn, and it MUST NOT inherit anything. The rule that falls out: the cost of a split is how much understanding the next agent has to rebuild.
...
mask measures are most of a run's round trips. None of those tools is slow: a
compile is 1.4 seconds, a silhouette view half a second, the whole measure phase
under ten. The tools are never the bill.
...
from 9.0s to 1.4s, a three-variant round from 11.0s to 4.4s, and Chromium is now
only needed for colour (`judge.mjs`), hero shots and the viewer.
